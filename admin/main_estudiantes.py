...
from modelos.modulo import User,TSession,session,Student,Pasantia,Configuracion
...
from herramientas.conversiones import null_string,convertir_pil_a_pixmap,convertir_a_json
...
class StackStudent():
    ...
    def listas(self):
        self.window.comboCarreras.clear()
        ...
        self.window.comboCarreras.addItem('Todas las Carreras',None)
        for i in carreras_json["carreras"]:
            self.window.comboCarreras.addItem(i,i)            
    ...
    def indice(self):
        ...
        self.window.btnInd.setText(f"{ind1}-{ind2 if ind2<self.all_data else self.all_data} de {self.all_data}")
    ...
    def header(self):
        ...
        #accion
        self.window.tabla_estudiantes.setColumnWidth(7, 100)
    ...
    def pag_tabla_estudiantes(self):
        ...
        for fila,estudiante in enumerate(estudiantes):
            ...
            tabla.setItem(fila,6,QTableWidgetItem(null_string(estudiante.telefono)))
    ...
    def get_data_students(self):
        search=self.window.searchStudent.text()
        self.all_data=len(session.scalars(self.get_data_students_all()).all())
        if not self.window.comboCarreras.currentData():
            return select(Student).where(
                                        or_(
                                            Student.primer_nombre.ilike(f"{search}%"),
                                            Student.primer_apellido.ilike(f"{search}%")
                                        )
                                        ).distinct().limit(self.tamano_pagina).offset(self.offset_count())
        else:
            return select(Student).join(
                                        Student.pasantias).where(
                                        Student.primer_nombre.ilike(f"%{search}%")).where(      
                                        Pasantia.carrera == self.window.comboCarreras.currentText()).distinct().limit(self.tamano_pagina).offset(self.offset_count())
            
    def get_data_students_all(self):
        search=self.window.searchStudent.text()  
        if not self.window.comboCarreras.currentData():
            return select(Student).where(
                                        or_(
    ...
    def exportar(self):
        ...
        if archivo:
            ...
            try:
                ...
                stmt=self.get_data_students_all()
                exportar_modelo_a_excel(session.scalars(stmt).all(),archivo,Student)
                
                QMessageBox.information(self.main, "Éxito", "La base de datos se exportó correctamente.")
                registrar_log(session=session, 
                              usuario=self.main.user_authenticated.username, 
                              accion="EXPORT",
                              tabla_afectada="Students", 
                              mensaje="Exportación de Datos de Estudiantes")
