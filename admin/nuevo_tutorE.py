...
from modelos.modulo import Tutor_Empresarial,Enterprise,session
...
class NewTutorE:
    ...
    def __init__(self,tutor=None):
        ...
        self.tutor=tutor
        self.window=cargar_ui("UI/new_tutor_empresarial.ui")
    ...
    def registrar(self):
        ...
        tutor=Tutor_Empresarial(primer_nombre=self.window.primernombre_input.text(),
                           ...
                           cargo=self.window.cargo_input.text())
        #empresa=session.query(Enterprise).where(Enterprise.id==self.window.empresa_input.currentData()).one_or_none()
        empresa=self.window.empresa_input.currentData()
        empresa.tutores.append(tutor)
    ...
    def actualizar(self):
        ...
        self.tutor.cargo=self.window.cargo_input.text()
        self.tutor.empresa=self.window.empresa_input.currentData()
        session.commit()
    ...
    def datos(self):
        self.window.label.setText("Editar Datos de Tutor Empresarial")
        ...
        self.window.cargo_input.setText(self.tutor.cargo)
        
        index=self.window.empresa_input.findData(self.tutor.empresa)
        self.window.empresa_input.setCurrentIndex(index)
    ...
    def subir_foto(self):
        ...
        if self.file_path:
            ...
            destino_dir = "resources/fotos_TutoresEmpresariales"
            if not os.path.exists(destino_dir):
                os.makedirs(destino_dir)
            
            # 3. Generar un nombre único para evitar duplicados
            extension = os.path.splitext(self.file_path)[1]
            nombre_archivo = f"tutor_empresarial_{datetime.now().strftime('%Y%m%d_%H%M%S')}{extension}"
